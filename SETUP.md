...
This guide walks through clone → install → `.env` → evals → **`agentlab ui`** (API + dashboard). A later section covers **[optional hosted deployment](#hosted-deployment-and-usage)** for teams who run the stack on a server.
...
3. **Next.js dashboard** — run history, diff viewer, project pages, landing (build with `npm install` under `agent_lab/agent_lab_ui`; optional **Spline** hero asset)
